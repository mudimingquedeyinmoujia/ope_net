...
import models
import utils
from test import eval_both_ope
import random
import myutils
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        if (epoch_val is not None) and (epoch % epoch_val == -1):
            if n_gpus > 1 and (config.get('eval_bsize') is not None):
                ...
                model_ = model
            with torch.no_grad():
                val_psnr, val_ssim = eval_both_ope(val_loader, model_)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='configs/train-div2k-OPEnet/final_ablation/train_edsr-OPE-abl-aa.yaml')
