...
class OPE_render_patch(nn.Module):
    """
    omega=pi
    use local ensemble
    """

    def __init__(self, max_freq, omega=math.pi):
        super(OPE_render_patch, self).__init__()
        self.max_freq = max_freq
        self.omega = omega
        self.lc_ope = LC_OPE(max_freq=self.max_freq, omega=self.omega)
    ...
    def query_rgb(self, feat, coord):
        ...
        pred = self.lc_ope(fourier_projection, rel_coord)  # N,bsize,3
        return pred
...
@register('OPE-net-a')
class OPE_net_a(nn.Module):
    def __init__(self, max_freq, srnet_spec=None):
        super(OPE_net_a, self).__init__()
        self.max_freq = max_freq
        self.render = OPE_render_intp(max_freq=max_freq, omega=math.pi)
        self.srnet = None
...
@register('OPE-net-b')
class OPE_net_b(nn.Module):
    def __init__(self, max_freq, srnet_spec=None):
        super(OPE_net_b, self).__init__()
        self.max_freq = max_freq
        self.render = OPE_render_patch(max_freq=max_freq, omega=0.5*math.pi)
        self.srnet = None
        if srnet_spec is not None:
            self.srnet = models.make(srnet_spec)
...
@register('OPE-net-c')
class OPE_net_c(nn.Module):
    def __init__(self, max_freq, srnet_spec=None):
        super(OPE_net_c, self).__init__()
        self.max_freq = max_freq
        self.render = OPE_render_patch(max_freq=max_freq, omega=math.pi)
        self.srnet = None
        if srnet_spec is not None:
            self.srnet = models.make(srnet_spec)
